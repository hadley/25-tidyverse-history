...
This article summarises almost 20 years of package development encompassing over 500 releases of 26 packages. So it's necessarily abbreviated. And my memory is fallible. So please let me know if you think I've forgotten something important!

```{r}
#| fig-label: fig-tidyverse-timeline
#| fig-cap: >
#|   A timeline of tidyverse (and adjacent) package releases.
package_release |>
  # filter(!type != "db") |>
  count(year, release) |>
  complete(year, release, fill = list(n = 0)) |>
...
While the tidyverse itself was named in 2016, the work that lead to it started much earlier, and its inextricably connected to the course of my career. @fig-first-release shows the releases of the tidyverse packages (and a couple of important precursors). From this diagram it's pretty clear that a lot of work happened before the tidyverse was named.

```{r}
#| fig-label: fig-first-releases
#| fig-cap: >
#|   Initial releases of tidyverse packages and important precusors.
#|   A vertical line indicates the release of the tidyverse package.
package_release |>
...
tibbles [started life](https://posit.co/blog/dplyr-0-4-0/) in dplyr 0.4.0 in early 2015, originally just called `tbl_df` with no suggestion as how to pronounce. tibbles started live in the dplyr package but were [extracted into their own package](https://posit.co/blog/tibble-1-0-0/) in 2016.
